...
class MinAvgTwoSlice {
    public static void main(String[] args) {
        ...
        System.out.println(minAvgTwoSlice.solution(new int[]{5, 6, 3, 4, 9})); //2
    }
    ...
    /*private int solution(int[] A) {
        ...
        double minAvg = divide(sum, A.length);
        int minIndex = 0;
    ...
    private int solution(int[] A) {
        ...
        for ( int i = 0; i < A.length - 1; i++ ) {
            double avg = divide(A[i ] + A[i + 1 ], 2);

            if (i < A.length - 2) {
                double avg3 = divide(A[i] + A[i + 1] + A[i + 2], 3);
                avg = Math.min(avg, avg3);
            }
